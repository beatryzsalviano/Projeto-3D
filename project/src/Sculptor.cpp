...
// Ativa o voxel na posi��o (x,y,z) (fazendo show = true) e atribui ao mesmo a cor atual de desenho
...
// Desativa o voxel na posi��o (x,y,z) (fazendo show = false)
...
void Sculptor::putEllipsoid(int xcenter, int ycenter, int zcenter, int rx,
                            int ry, int rz){
  // Ativa Voxels que satisfazem a equa��o do elips�ide
  for (int i = 0; i < nx; i++){
    for (int j = 0; j < ny; j++){
      for (int k = 0; k < nz; k++){
...
void Sculptor::cutEllipsoid(int xcenter, int ycenter, int zcenter, int rx,
                            int ry, int rz){
  // Desativa Voxels que satisfazem a equa��o do elips�ide
  for (int i = 0; i < nx; i++){
    for (int j = 0; j < ny; j++){
      for (int k = 0; k < nz; k++){
        float normalizedX = static_cast<float>(i - xcenter) / rx;
...
// Fun��o que cria o arquivo .off
void Sculptor::writeOFF(char const *filename){
  std::ofstream outfile(filename);
  // Se o arquivo n�o for gerado gera um aviso
  if (!outfile.is_open()){
  ...
  // Calcula o n�mero de Voxels ativos (show == true)
  for (int i = 0; i < nx; i++){
    for (int j = 0; j < ny; j++){
      for (int k = 0; k < nz; k++){
        if (v[i][j][k].show) {
  ...
  // Esvreve o n�mero de v�rtices, faces, e arestas (0)
  outfile << numVertices << " " << numFaces << " 0" << std::endl;

  // Escreve as coordenadas dos v�rtices
  for (int i = 0; i < nx; i++){
    for (int j = 0; j < ny; j++){
      for (int k = 0; k < nz; k++){
        if (v[i][j][k].show){
